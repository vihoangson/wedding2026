...
if (isset($_GET['action']) && $_GET['action'] === 'get_data') {
    ...
    $rsvp = file_exists($DATA_FILE) ? json_decode(file_get_contents($DATA_FILE), true)['rsvp_list'] : [];
    $comments = file_exists($COMMENTS_FILE) ? json_decode(file_get_contents($COMMENTS_FILE), true)['comments'] : [];

    $confirmed = count(array_filter($rsvp, fn($x) => ($x['attend'] ?? '') === 'yes'));
    $declined = count(array_filter($rsvp, fn($x) => ($x['attend'] ?? '') === 'no'));
    $guests = array_sum(array_column($rsvp, 'guests', null));

    die(json_encode([
        'rsvp' => $rsvp,
        'comments' => $comments,
        'stats' => [
            'total_rsvp' => count($rsvp),
            'confirmed' => $confirmed,
            'declined' => $declined,
            'total_guests' => $guests
        ]
